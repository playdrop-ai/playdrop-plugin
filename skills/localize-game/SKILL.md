...
For requested images, localize visible copy and approved titles, including number formatting. For incidental promotional scores, ungrouped digits are acceptable when the value stays clear; do not spend repeated generation passes forcing a thousands separator. Keep runtime number formatting locale-aware. Reuse text-free icons and unchanged-title heroes. Follow `../make-marketing-screenshots/SKILL.md` and the identity rules in `../make-listing/SKILL.md`; preserve actual game content and avoid unnecessary redesign. Use the original artwork as the edit source; accepted translated images can serve as style references, so visual changes do not accumulate across locales.
