...
6. Subscribe to `sdk.host.onPhaseChange(...)`. Mount the Editor when the phase becomes `editor`, dispose or hide its controls when the phase becomes `play` or `preview`, and call `sdk.host.editorReady()` again after every Editor mount. Studio changes phase on the existing iframe, so an Editor that only checks the initial phase is invalid.
