...
const validator = fileURLToPath(new URL("../scripts/validate-social-package.mjs", import.meta.url));
...
async function makeFixture(frameRate = 30) {
  ...
  run("ffmpeg", [
    ...
    "lavfi",
    "-i",
    `color=c=black:s=1920x1080:r=${frameRate}:d=30`,
    "-f",
    "lavfi",
    "-i",
  ...
  run("ffmpeg", [
    ...
    "lavfi",
    "-i",
    `color=c=black:s=1080x1920:r=${frameRate}:d=12`,
    "-f",
    "lavfi",
    "-i",
    "anullsrc=channel_layout=stereo:sample_rate=48000",
...
test("rejects video below the required frame rate", async () => {
  const root = await makeFixture(24);
  await writeFile(
    join(root, "manifest.json"),
    JSON.stringify({
      destinationUrl: "https://www.playdrop.ai/@studio/game",
  ...
  const result = spawnSync("node", [validator, root], { encoding: "utf8" });
  assert.equal(result.status, 1);
  assert.match(result.stderr, /must be 30 fps/);
});
