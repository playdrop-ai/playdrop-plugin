...
Write `playtest-evidence.json` after the final successful check. This is validation evidence, not a design document. Include per entry: `url`, `surface`, `captures`, `actions`, `statesObserved`, `consoleErrors`, `webglRenderer`, `environment`, `checkedAt`; and for a new game the top-level `proof` object:
...
Each proof moment needs its own capture from the current non-final playtest entry. A staged capture mode, debug hook, source-code path, or prose claim is not playtest proof. The upload preflight validates the exact manifest shape and its error text prints the full expected schema: run the upload early and fix exactly what it names rather than guessing.
