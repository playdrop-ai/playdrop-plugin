...
- **In-game AI dialogue or text/JSON:** Use a declared game server and server `playdrop.ai.text` / `playdrop.ai.json`; the app creator pays. Browser `sdk.ai.text.createTask` / `sdk.ai.json.createTask` reject hosted app tokens with `app_ai_generation_disabled`. Browser media generation remains a separate host-confirmed flow. Read `../../references/tech/game-servers.md` before choosing the architecture.
...
1. Keep the first version focused, fun, and shippable. When the core interaction is uncertain, prove a small playable version before polish; art-first or parallel art and code suit familiar or visually led games. If live AI is the core interaction, first prove one real request and completed response through the declared game server and hosted test player before producing artwork or the full interface. SDK declarations or a successful build do not prove that capability works.
