...
- Use the creator surface as the primary surface when the prompt gives one, unless it is a terrible fit for the requested game. Inside the app entry in `catalogue.json`, enable every honestly supported `surfaceTargets` surface, set `primarySurface`, and add one complete `playtestTapes` entry per enabled surface. These fields are required for every newly created game even though old catalogue content remains compatible without them.
