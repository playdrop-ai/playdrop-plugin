...
| Gemini | Consumer app custom remote MCP availability unverified | Do not claim supported |
| Antigravity | `variants/antigravity/playdrop` | Manual install; curated store is separate |
| Grok Bot | Custom connector availability depends on product/account | Do not equate with Grok Build |
| Grok Build | `variants/grok/playdrop` | Repo marketplace, no public listing required |
| Cursor | Root portable package where supported, or custom remote MCP URL | Optional marketplace submission |
| Lovable | Add a custom MCP chat connector with URL and OAuth | No listing required for private connection |
| Replit | Add MCP server, URL, then Test & save | No listing required; optional one-click install link |
...
Local read-only acceptance passed with Codex 0.153.4's `mcp_2026_07_28` feature and
Claude Code 2.1.270's `MCP_SDK_GENERATION=v2`, using programmatic fixture OAuth.
...
These checks do not establish PlayDrop browser OAuth UX or production readiness.
...
Lovable and Replit explicitly support custom remote MCP by URL. Replit documents automatic OAuth DCR.
