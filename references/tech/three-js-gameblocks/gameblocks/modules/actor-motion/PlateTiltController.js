...
export class PlateTiltController {
  constructor({ maxTiltRadians = 0.13, tiltLag = 0.1, basis = DEFAULT_WORLD_BASIS }) {
  ...
  move({ forward = 0, backward = 0, left = 0, right = 0, deltaSeconds = 1 / 60 }) {
    const targetForward =
      this.basis.controlSignal("clockWise", forward) + this.basis.controlSignal("counterClockWise", backward);
    const targetRight =
      this.basis.controlSignal("clockWise", right) + this.basis.controlSignal("counterClockWise", left);
