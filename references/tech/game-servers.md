...
Use this capability for every new multiplayer or server-authoritative game and for live in-game AI dialogue or generated text/JSON. It is the only supported way for a game to run custom server-side code. Prefer a client-only game only when it needs no realtime multiplayer, trusted server rules, server-side AI, shared authoritative state, or server-owned persistent data.
...
- **AI generation:** Import `playdrop` from `@playdrop/server` and use `playdrop.ai.text.createTask(...)` or `playdrop.ai.json.createTask(...)` for live in-game dialogue or generated text/JSON. Media modalities are available on the same server API. These are asynchronous tasks; use `playdrop.ai.tasks` to retrieve the completed response. The app creator pays and owns the result unless ownership is assigned with `{ owner: client.auth }` using the verified player. Test-player generation stays creator-owned and uses test mode. Browser `sdk.ai.text.createTask` and `sdk.ai.json.createTask` reject hosted app tokens with `403 app_ai_generation_disabled`; there is no enablement flag. Browser media generation remains a separate host-confirmed quote flow. Server AI does not provide arbitrary HTTP access or asset-pack creation.
- **Generated private media:** Once a server generation task is `DONE`, call server `playdrop.ai.tasks.downloadFile(task.id, { role: 'primary' })` to obtain its `Blob`. Send its bytes and MIME type through the room only to the intended authenticated client; the browser reconstructs a `Blob` and uses an object URL. Browser `sdk.ai.tasks.downloadFile` and public catalogue downloads cannot retrieve creator-owned private output. Keep it private and keep server credentials on the server. Limit each speech input to 40 words; split longer dialogue before creating speech tasks. See the installed `@playdrop/server` README for the delivery example.
...
When live AI is the core interaction, prove one real request and completed response through the declared server and a hosted test player before producing artwork or the full interface. Pass the verified test player as `{ owner: client.auth }` so generation uses test mode. For voice, also download the completed file, deliver it through the room, and verify audible browser playback after the player's audio gesture. A `DONE` task alone does not prove voice delivery. Check the actual response, not just SDK declarations, a successful build, or a queued task. If the supported path fails, retain the exact error and stop before investing in the rest of the game.
