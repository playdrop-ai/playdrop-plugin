...
- Respect `sdk.host.phase`: preview should show a live representative scene, play should accept input, and an optional Editor should mount only in `editor` phase and call `sdk.host.editorReady()` when usable. Mount the Editor before reading `sdk.creator`; staged Editor validation has no creator APIs because the version is not finalized yet. Read Tweaks, Notes, or assets lazily when the creator uses those actions.
